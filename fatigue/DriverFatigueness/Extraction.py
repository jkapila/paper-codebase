...
from models.Process import modelLoader, testEyeOnModel, testYawnOnModel,testPerclosOnModel,testDrowsieness
from parameters.Parameters import TestEnvParameters
from utils.Utils import resizing, stream_counter, stream_init
...
resize_image_px = param.VIDEO_RESOLUTION
font = cv2.FONT_HERSHEY_COMPLEX

# Making and loading CNN Models
modelLoader(epochs_eye=epochs_eye,epochs_yawn=epochs_yawn,
            saved_models=[param.USE_SAVED_MODEL_EYE, param.USE_SAVED_MODEL_PERCLOS,
                          param.USE_SAVED_MODEL_YAWN, param.USE_SAVED_MODEL_DROWSINESS],
            model_path='{}/models'.format(param.DIR_PATH))

# Setting HARR Cascades from Opencv Location
haar_path = param.HAAR_PATH
if len(haar_path) == 0:
    haar_path = str(os.getcwd()).__add__('/data/haarcascades/')

# Loading Basic Haar Cascades
face_cascade = cv2.CascadeClassifier(haar_path.__add__('haarcascade_frontalface_alt.xml'))
...
def detectEyePerclos(image):
    val = testPerclosOnModel(np.array(image, dtype='float32'))
    print("Prediction for Perclos", val[0], 'Is Perclos(>80%)?:',val[0] >= param.PROB_EYE)
    return val[0] >= param.PROB_PERCLOS

def detectDrowsiness(eye_Counter, perclos_counter, yawn_counter):
    val = testDrowsieness(eye_Counter, perclos_counter, yawn_counter)
    print("Prediction for Drowsy", val[0], 'Is Drowsy?:', val[0] >= param.PROB_EYE)
    return val[0] >= param.PROB_DROWSY
...
while cap.isOpened():
    ...
    for (x, y, w, h) in faces:
        ...
        # drowsieFalg = detectDrowsines(blink_counter_stream,yawn_counter_stream)
