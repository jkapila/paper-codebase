...
def perclosCNNModel(batch_size = 30,epochs = 20,optimizer = 'sgd',lr = 'default'):
    ...
    # print shape of data while perclosModel is building
    ...
    # global perclosModel
    perclosModel = Sequential()
    # print("\nAdding Level 1")
    # perclosModel.add(Conv2D(32, (3, 3), padding='same', input_shape=(img_samples, img_channels, img_rows, img_cols)))
    perclosModel.add(Conv2D(32, (3, 3),
                     # padding='same',
                     data_format="channels_last",
                     input_shape=(img_rows, img_cols, img_channels)))

    perclosModel.add(Activation('relu'))

    # print("\nAdding Level 2")
    perclosModel.add(Conv2D(24, (3, 3)))
    perclosModel.add(Activation('relu'))
    perclosModel.add(MaxPooling2D(pool_size=(2, 2)))
    perclosModel.add(Dropout(0.25))

    # print("\nAdding Level 3")
    perclosModel.add(Conv2D(64, (3, 3), padding='same'))
    perclosModel.add(Activation('relu'))

    # print("\nAdding Level 4")
    perclosModel.add(Conv2D(64, (3, 3)))
    perclosModel.add(Activation('relu'))
    perclosModel.add(MaxPooling2D(pool_size=(2, 2)))
    perclosModel.add(Dropout(0.25))

    # print("\nAdding Level 5")
    perclosModel.add(Flatten())
    perclosModel.add(Dense(512))
    perclosModel.add(Activation('relu'))
    perclosModel.add(Dropout(0.5))
    perclosModel.add(Dense(nb_classes))
    perclosModel.add(Activation('sigmoid'))

    # let's train the perclosModel using SGD + momentum (how original).
    if optimizer == 'sgd':
    ...
    perclosModel.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])

    perclosModel.fit(X_train, Y_train, batch_size=batch_size,
              epochs=epochs, verbose=2, validation_data=(X_test, Y_test))

    score = perclosModel.evaluate(X_test, Y_test, verbose=1)
